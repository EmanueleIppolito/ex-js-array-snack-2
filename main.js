...
const longBooksTitles = longBooks.map(titolo => titolo.title);

console.log(longBooks)

console.log(longBooksTitles)

books.forEach(titolo => console.log(titolo.title))
...
const agesSum = ages.reduce((acc, ages) => acc + ages, 0)
...
function getBooks(ids){
    const requests = ids.map(id => fetch(`http://localhost:3333/books/${id}`)
    .then(res => res.json()))
